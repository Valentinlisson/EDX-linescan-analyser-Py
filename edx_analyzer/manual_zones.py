...
class ManualZoneDialog(ctk.CTkToplevel):
    """Prompts for a zone name, precise start/end position and a color.
    `on_confirm` is called with the resulting zone dict when saved."""

    def __init__(self, master, x_min, x_max, initial_name="", initial_color=DEFAULT_ZONE_COLOR, on_confirm=None):
...
class ManualZoneManagerWindow(ctk.CTkToplevel):
    ...
    def _refresh(self):
        ...
        for idx, z in enumerate(self.zones):
            ...
            desc = f"{z['name']}   [{z['start']:.2f} → {z['end']:.2f}]"
    ...
    def _edit(self, idx):
        ...
        ManualZoneDialog(self, z["start"], z["end"], initial_name=z["name"], initial_color=z.get("color", DEFAULT_ZONE_COLOR), on_confirm=on_confirm)
